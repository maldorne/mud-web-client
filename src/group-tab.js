var GroupTab = function (o) {
  ...
  var draw = function () {
    ...
    for (i = 0; i < m.length; i++) {
      ...
      j(o.id + ' .list').append(
        '\
          ...
          classes[mem.info.class] +
          ', Level ' +
          mem.info.lvl +
          '</div>\
							<div class="group-chat"></div>\
							<div class="group-bars">\
								<img class="group-bar group-healthbar" src="' +
          path +
          'healthbar.png" style="width:' +
          Math.floor((mem.info.hp * 186) / mem.info.mhp) +
          'px">\
								<img class="group-bar group-manabar" src="' +
          path +
          'manabar.png" style="width:' +
          Math.floor((mem.info.mn * 186) / mem.info.mmn) +
          'px">\
								<img class="group-bar group-movebar" src="' +
          path +
          'movebar.png" style="width:' +
          Math.floor((mem.info.mv * 186) / mem.info.mmv) +
          'px">\
								<img class="group-bar group-expbar" src="' +
          path +
          'expbar.png">\
							</div>\
							<div class="group-stats">\
								<span class="group-stat group-hp">' +
          mem.info.hp +
          '</span> / <span class="group-maxhp">' +
          mem.info.mhp +
          '</span><br>\
								<span class="group-stat group-mn">' +
          mem.info.mn +
          '</span> / <span class="group-maxmn">' +
          mem.info.mmn +
          '</span><br>\
								<span class="group-stat group-mv">' +
          mem.info.mv +
          '</span> / <span class="group-maxmv">' +
          mem.info.mmv +
          '</span><br>\
    ...
    if (o.noart) j(o.id + ' .noart div').css({ left: 20 });
  };
  ...
  var aff = function () {
    ...
    for (var i = 0; i < affs.length; i++) {
      if (affs[i].gives)
        t.append(
          '<div class="group-one-aff tip" title="Gives ' +
            affs[i].gives +
            ' to ' +
            affs[i].to +
            '">' +
  ...
  var selected = function () {
    if (!j(o.id + ' .active').length) return null;
