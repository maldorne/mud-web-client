var ControlPanel = function () {
  ...
  var loadProfiles = function () {
    ...
    if (pref.profiles) {
      for (var p in pref.profiles) {
        ...
        if ((Config.clean || Config.solo) && Config.host != P.host) continue;
        var i = d.index('host', P.host);
        j(id + ' .gamelist .profiles').append(
          '<a class="profile-link" host="' +
            d[i].host +
            '" port="' +
            d[i].port +
            '" thumb="' +
  ...
  var loadMudconnectList = function () {
    if (Config.solo || Config.clean || touch) return;
    ...
    j.get('/app/xml/mudconnect.xml', function (d) {
      ...
      for (var g = 0; g < tmc.length; g++) {
        ...
        if (name.length && host.length && port.length)
          j(id + ' .gamelist .tmc').append(
            '<a style="display:none" class="tmc-link game-link" host="' +
              host +
              '" port="' +
              port +
              '" thumb="' +
    ...
    for (g = 0; g < d.length; g++) {
      try {
        ...
        d[g].img =
          '/' +
          el
            .match(/"(images\\\/.+?\.(png|gif|jpg))"/i)[1]
            .replace(/\\\//g, '/');

        j(id + ' .sitelist').append(
          '<a class="game-link" data="' +
            g +
            '" host="' +
            d[g].host +
            '" port="' +
  ...
  var loadChat = function () {
    ...
    for (var i = 0; i < chan.length; i++)
      j(id + ' .chatlist').append(
        '<a class="chat-link" data="' +
          chan[i] +
  ...
  j(id).on('click', ' .game-link, .profile-link', function (e) {
    ...
    if (profile) t.attr('profile', profile);
    ...
    t.html(
      '<div class="left game-blurb" style="padding: 4px 18px 0px 4px"><img class="game-thumb" src="' +
        thumb +
    ...
    if (window.user.id && pref) {
      ...
      else if (pref.sitelist && pref.sitelist[name])
      ...
      if (G) {
        ...
        for (var n = 0; n < G.macros.length; n++) {
          ...
          t.append(
            '<div>\
					<input type="text" style="width: 100px" placeholder="macro name" value="' +
              i[0] +
              '"> \
					<input type="text" placeholder="commands to send" value="' +
              i[1] +
              '"> \
					<i class="icon-' +
              (i[2] ? 'check' : 'unchecked') +
        ...
        for (var n = 0; n < G.triggers.length; n++) {
          ...
          t.append(
            '<div>\
					<input type="text" style="width: 100px" placeholder="trigger phrase" value="' +
              i[0] +
              '"> \
					<input type="text" placeholder="response" value="' +
              i[1] +
              '"> \
					<i class="icon-' +
              (i[2] ? 'check' : 'unchecked') +
    ...
    if (G && G.settings)
      for (var s = 0; s < G.settings.length; s++)
        if (!G.settings[s].value)
          j(id + ' #settings ' + '#' + G.settings[s].id)
            .removeClass('icon-check')
            .addClass('icon-unchecked');
        else if (G.settings[s].value)
          j(id + ' #settings ' + '#' + G.settings[s].id)
  ...
  if (param('profile')) {
    j(id + ' .profile-link').each(function () {
      if (j(this).attr('profile') == param('profile')) j(this).click();
    });
  } else if (param('host')) {
    j(id + ' .game-link').each(function () {
      if (j(this).attr('host') == param('host')) j(this).click();
    });
  } else if (j(id + ' .game-link:first').length)
  ...
  j(id).on('click', '.save', function () {
    ...
    if (!profile) {
      if (!pref.sitelist) pref.sitelist = {};
      ...
      if (!pref.profiles) pref.profiles = {};
    ...
    j(id + ' #macros .scroll div').each(function () {
      ...
      if (!(a = j(this).find('input:first').val())) return;

      if (!(b = j(this).find('input:nth-child(2)').val())) return;

      c = j(this).find('.icon-check').length;
      d = j(this).find('.icon-star').length;

      if (profile) pref.profiles[profile].macros.push([a, b, c, d]);
      else pref.sitelist[name].macros.push([a, b, c, d]);
    });
    ...
    j(id + ' #triggers .scroll div').each(function () {
      ...
      if (profile) pref.profiles[profile].triggers.push([a, b, c]);
      else pref.sitelist[name].triggers.push([a, b, c]);
    });
    ...
    j(id + ' #settings .scroll i').each(function () {
      ...
      if (profile) pref.profiles[profile].settings.push(a);
      else pref.sitelist[name].settings.push(a);
    });

    j.post(
      '?option=com_portal&task=set_pref',
      { pref: stringify(pref) },
      function () {
        new Modal({
    ...
    if (Config.TriggerHappy) Config.TriggerHappy.init();
      ...
      buttons: [
        {
          ...
          click: function () {
            ...
            if (!(v = j('#modal .modal-body input').val())) return;

            if (!user.pref.profiles) user.pref.profiles = {};
  ...
  var closeChat = function () {
    ...
    if (nice) nice.remove();
    if (user.channel) delete user.channel;
  };

  j(id).on('click', '.chat-link', function (e) {
    var channel = j(this).attr('data');

    if (mychannel && mychannel == channel) return;

    if (mychannel) closeChat();

    j(id + ' .gamepanel').html(
      '<div class="chat-panel">\
		<!--<div class="chat-title right" style="color:#01c8d4;opacity:0.6">' +
        channel.toUpperCase() +
        '</div>-->\
		<div class="nice chat-main chat-' +
        channel +
        ...
        if (mychannel == i[1].channel)
          c.append(
            '<span style="opacity: 0.6">' +
              i[0].substr(11, 5) +
              '</span> <span style="color: #01c8d4;opacity:0.7">' +
              i[1].name +
              ': </span>' +
              linkify(i[1].msg) +
              '<br>'
          ...
          c.append(
            '<span style="opacity: 0.6">' +
              i[0].substr(11, 5) +
              '</span> <span style="color: #FFEB9E;opacity:0.6">' +
              i[1].name +
