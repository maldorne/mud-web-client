...
var MXP = function () {
  ...
  var process = function (t) {
    ...
    if (!mxp && !t.match(/\x1b\[[0-7]z/)) return t;

    t = t.replace(/\r/g, '');

    if (t.has('<VERSION>')) t = t.replace('\x1b[1z<VERSION>\x1b[7z', '');
  ...
  var frames = function (t) {
    if (t.match(/\x1b\[[1-7]z<frame/i)) {
      ...
      j('#mxpf .iframe').each(function () {
        ...
        if (!action && !close && !eof) action = 'open';
        else action = 'eof';

        if (action.toLowerCase() == 'close') close = 1;

        if (action.toLowerCase() == 'open') open = 1;
        ...
        if (align && align.length) {
          if (align.has('top')) css.top = 0;
          if (align.has('bottom')) css.bottom = 0;
          if (align.has('left')) css.left = 0;
          if (align.has('right')) css.right = 0;
        }

        if (css.width || css.height || css.left || css.top) css.pos = 1;
        else css = null;

        if (window[n] || j('.' + n).length) {
          if (close) j('.' + n).remove();
          else if (eof || j('.' + n).length) {
            if (j('.' + n + ' .out').length)
              /* need a better way to empty different elements */
              j('.' + n + ' .out').empty();
            else j('.' + n + ' .content').empty();
          } else if (action == 'open') {
            try {
    ...
    if (t.match(/\x1b\[[1-7]z<dest/i)) {
      ...
      j('#mxpf .dest').each(function () {
        ...
        } else if (Config.ScrollView) Config.ScrollView.add(msg);
      });
  ...
  j('body').on('click', '.mxp', function (evt) {
    ...
    if (href) {
      if (href.has('|')) multi(href, this);
      else if (href == '#') Config.socket.write('');
      else Config.socket.write(href);
    } else Config.socket.write(j(this).text());

    return false;
  });

  j('body').on('click', function (evt) {
    if (!j(this).is('a')) j('.mxp-dropdown').remove();
  });
...
} else log('MXP disabled in profile or game preferences.');
