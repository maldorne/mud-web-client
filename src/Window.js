var Window = function (o) {
  ...
  if (Config.device.touch || param('gui')) o.handle = '.none';
  ...
  var resize = function () {
    ...
    j(id + ' .tab-pane').each(function () {
      var w = j(this).parent().width(),
        h = j(this).parent().height(),
        ch = h;

      if (j(this).find('.footer')) ch -= j(this).find('.footer').height() + 4;
  ...
  var init = function (o) {
    ...
    if (o.transparent) o.handle = '.content';

    j(id).click(front);

    draggable();

    if (Config.Toolbar) j(id + ' .handle').dblclick(hide);
    ...
    if (o.nofront) j(id).addClass('nofront');

    if (o.nofade) j(id).addClass('nofade');

    j(id + ' .ui-resizable-handle').css('z-index', j(id).css('z-index'));

    if (id == '#scroll-view' && default_pos) {
      if (Config.Device.lowres) win.maximize();
      else j(id).center();
    }

    if (Config.collapse.has(id)) collapse();

    for (var i = 0; i < o.tabs.length; i++) tab(o.tabs[i]);
  ...
  var collapse = function () {
    if (j(id + ' .content').hasClass('hidden')) {
      ...
      j(id + ' .icon-plus')
        ...
        .attr('title', 'Collapse this window.');
  ...
  var front = function (save) {
    if (j(id).hasClass('nofront')) return;
    ...
    j(id)
      ...
      .each(function () {
        ...
        if (!j(this).hasClass('nofade')) j(this).css('opacity', 0.3);
      });

    if (!Z.length) return log('solo window. no front needed');
    ...
    for (var i = 0; i < Z.length; i++) j(Z[i].id).css('z-index', minZ + i);
  ...
  var savepos = function () {
    ...
    if (!user.pref.win[view_id]) user.pref.win[view_id] = {};
  ...
  var getpos = function (o) {
    if (!save || !user.id || !user.pref || !user.pref.win) return 1;
    ...
    if (pref.offset.left < 0) pref.offset.left = 0;

    if (pref.offset.left > j(window).width())
      pref.offset.left = j(window).width() - pref.width;

    if (pref.offset.top < 0) pref.offset.top = 0;
