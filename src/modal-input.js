var ModalInput = function (o) {
  ...
  var close = function (send) {
    ...
    if (send !== true) Config.Socket.write(o.abort);
  ...
  if (o.close && o.close == 1) return close();
  ...
  if (o.text) o.text = colorize.strip(o.text).replace(/\r/g, '');
  ...
  j('body').append(
    '\
		<div class="modal ' +
      (o['class'] || '') +
      ' fade me-modal"><div class="modal-dialog"><div class="modal-content">\
			<div class="modal-header">\
				<button type="button" class="close me-dismiss">×</button>\
				<h3>' +
      (o.title || 'Multi-Line Input') +
      '</h3>\
			</div>\
      ...
      '\
			...
			<div class="modal-footer">\
				<button class="btn btn-primary kbutton me-dismiss">' +
      (o.closeText || 'Cancel') +
      '</button>\
  ...
  if (o.attr && (o.attr == 'reset' || o.attr == 'change')) {
    ...
    j('.modal input').on('keyup', function () {
      var a = j('.modal .me-input').val(),
        b = j('.modal .me-pass2').val();

      if (a.length > 5 && b.length > 5 && a == b)
        j('.modal .me-send').removeClass('disabled');
      else j('.modal .me-send').addClass('disabled');
    });
  }
  ...
  if (o.buttons) {
    ...
    for (var i = 0; i < o.buttons.length; i++) {
      var b = o.buttons[i];
      var cls =
        !b.click && !exists(b.send)
          ? ' me-send'
          : ''; /* button without custom handling is the send one */
  ...
  if (o.sendText) j('.modal .me-send').text(o.sendText);
...
Event.listen('gmcp', function (d) {
  ...
  if (!d.start) console.log(d);

  if (!d.start || !d.start('ModalInput')) return d;
