    ...
    Config.MistyBars = new MistyBars({
      process: function (d) {
        try {
          ...
          if (!key.start('ch.points')) return d;
          ...
          cs = {
            /* level: 210, enl: 3000, tnl: 1000, state: 3, pos: "Standing", */
  ...
  if (a == 'login') {
    try {
      FB.getLoginStatus(function (resp) {
        if (resp.status == 'connected') Facebook('statusChange', resp);
        else
          FB.login(
            function (resp) {
...
Event.listen('gmcp', function (d) {
  if (!d || !d.start || !d.start('game.info')) return d;
